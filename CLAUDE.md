...
- **시작 시 `docs/INDEX.md`를 먼저 읽어** 어떤 문서를 봐야 하는지 찾아간다. (역할별 읽는 순서·전체 목록)
- **새 문서를 만들거나 이름을 바꿀 때마다 `docs/INDEX.md` 색인 표에 한 줄을 *반드시* 추가/갱신한다.** 색인에 없는 문서는 만든 게 아니다.
- 단일 진실원천: Event 스키마는 `docs/event-schema.md` 하나뿐 — 다른 문서에 복제 금지, 변경은 이 파일 PR로만.
