@echo off
REM ============================================================
REM  AgenTrace.exe build script (Windows)
REM  - Double-click this file, or run from cmd.
REM  - Output: dist\AgenTrace.exe  (run with no args -> native window; browser fallback)
REM ============================================================
setlocal
...
python -m PyInstaller --noconfirm --onefile --name AgenTrace --icon "packaging\AgenTrace.ico" --collect-all webview --add-data "clfx\web\static;clfx\web\static" packaging\launcher.py
if errorlevel 1 (
...
if not exist "dist\AgenTrace.exe" (
  echo [ERROR] dist\AgenTrace.exe not produced. See log above.
  pause
  exit /b 1
)
echo.
echo [clfx] Done. Output: %CD%\dist\AgenTrace.exe
echo [clfx] Built artifact:
dir "dist\AgenTrace.exe" | findstr AgenTrace.exe
echo        Double-click AgenTrace.exe to launch (native window with the scan screen; browser fallback).
pause
endlocal
