---
description: tmux panel 1에 개발 세션 기동 (claude 또는 codex)
...
```bash
# 현재 창에 pane이 2개 이상이면 panel 1이 이미 떠있음 (%ID 캐시 의존 X)
PID="$(bash .claude/bin/resolve-pane.sh panel1 2>/dev/null || true)"
P0="$(bash .claude/bin/resolve-pane.sh panel0 2>/dev/null || true)"
if [ -n "$PID" ] && [ "$PID" != "$P0" ] && tmux list-panes -a -F '#{pane_id}' 2>/dev/null | grep -qx "$PID"; then
  echo "panel 1 already alive: $PID"; exit 1
fi
```

중단 시 사용자에게 안내:
> "panel 1이 이미 열려있다. 먼저 `tmux kill-pane -t $(bash .claude/bin/resolve-pane.sh panel1)` 로 종료한 뒤 다시 실행해라."
...
tmux set-option -p -t "$(cat .harness/panel0.id)" @fa_role panel0   # %ID-비의존 라우팅 태그
```
...
tmux set-option -p -t "$(cat .harness/panel1.id)" @fa_role panel1   # %ID-비의존 라우팅 태그
```
